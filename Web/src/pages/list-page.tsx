...
const defaultFilters: ListingFilters = { query: '', type: '', property: '', minPrice: '', maxPrice: '', bedrooms: '', radius: '5' };
const containerVariants = { hidden: { opacity: 1 }, visible: { opacity: 1, transition: { staggerChildren: 0.08 } } };
...
function distanceInKm(a, b) {
	...
	const calculation = Math.sin(latDistance / 2) ** 2 + Math.cos(firstLat) * Math.cos(secondLat) * Math.sin(lonDistance / 2) ** 2;
...
function ListPage() {
	...
	const filteredPosts = useMemo(() => postsData.filter(post => {
	...
	const resetFilters = () => { setFilters(defaultFilters); setSelectedLocation(null); };

	return (
		<div className="min-h-screen bg-[#f7f8fb]">
			<nav className="relative z-50 border-b border-gray-200 bg-white px-4 py-4 md:px-10 xl:px-16">
				<div className="mx-auto flex max-w-[1600px] items-center justify-between">
					<Link to="/" className="flex items-center"><BuildingIcon className="h-8 w-8 text-primary" /><span className="ml-3 text-lg font-bold uppercase tracking-wider text-gray-900">HOC Living Faro</span><span className="ml-2 hidden text-xs uppercase tracking-widest text-gray-400 sm:inline">Real Estate</span></Link>
					<div className="flex items-center gap-4 md:hidden"><button onClick={() => setIsFilterOpen(!isFilterOpen)} aria-label="Toggle filters"><FilterIcon className="h-6 w-6 text-gray-800" /></button><button onClick={() => setIsNavOpen(!isNavOpen)} aria-label="Toggle navigation"><HamburgerIcon className="h-6 w-6 text-gray-800" /></button></div>
					<div className={`${isNavOpen ? 'flex' : 'hidden'} absolute left-0 top-16 w-full flex-col gap-3 bg-white p-4 shadow-md md:static md:flex md:w-auto md:flex-row md:gap-6 md:p-0 md:shadow-none`}>{links.map(text => <Link to="/listings" className="text-sm text-gray-600 transition hover:text-gray-950" key={text}>{text}</Link>)}</div>
					<div className="hidden gap-2 md:flex"><Button variant="outline">Contact us</Button><Button className="bg-accent text-white hover:bg-accentHover">Join</Button></div>
				</div>
			</nav>
			...
			<main className="mx-auto grid max-w-[1600px] grid-cols-1 gap-6 px-4 py-6 md:px-10 xl:grid-cols-[minmax(0,1fr)_430px] xl:px-16">
				<div className="min-w-0">
					<div className={`${isFilterOpen ? 'block' : 'hidden'} md:block`}><Filter filters={filters} onChange={setFilters} onReset={resetFilters} resultCount={filteredPosts.length} hasMapLocation={Boolean(selectedLocation)} /></div>
